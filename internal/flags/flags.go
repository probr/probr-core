...
func addVerboseFlag(flagSet *flag.FlagSet) {
	config.Vars.Verbose = flagSet.Bool("v", false, "Display extended version information")
}

func addAllFlag(flagSet *flag.FlagSet) {
	config.Vars.AllPacks = flagSet.Bool("all", false, "Include all installed packs, not just those specified within the provided config")
}
